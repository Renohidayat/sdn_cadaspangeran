                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header border-bottom">
                            <h4 class="card-title">Data Guru <a href="{{ route('backend-pengguna-pengajar.create') }}" class="btn btn-primary">Tambah</a></h4>
                        </div>
                        <div class="card-body">
                            @if (session('success'))
                                ...
                                <tbody>
                                    @foreach ($data as $key => $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->telepon }}</td>
                                            <td>
                                                <a href="{{ route('backend-pengguna-pengajar.edit', $item->id) }}" class="btn btn-sm btn-info">Edit</a>
                                                <form action="{{ route('backend-pengguna-pengajar.destroy', $item->id) }}" method="POST" style="display: inline-block;">
                                                    @csrf
                                                    @method('DELETE')
