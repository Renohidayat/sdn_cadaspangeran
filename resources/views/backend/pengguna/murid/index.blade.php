                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header border-bottom">
                            <h4 class="card-title">Data Siswa <a href="{{ route('backend-pengguna-murid.create') }}" class="btn btn-primary">Tambah</a></h4>
                        </div>
                        <div class="card-body">
                            @if (session('success'))
                                ...
                                <tbody>
                                    @foreach ($data as $key => $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->status }}</td>
                                            <td>
                                                <a href="{{ route('backend-pengguna-murid.edit', $item->id) }}" class="btn btn-sm btn-info">Edit</a>
                                                <form action="{{ route('backend-pengguna-murid.destroy', $item->id) }}" method="POST" style="display: inline-block;">
                                                    @csrf
                                                    @method('DELETE')
